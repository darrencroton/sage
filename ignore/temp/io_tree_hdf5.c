...
void load_tree_table_hdf5(int filenr)
{
  ...
  if (status != EXIT_SUCCESS)
  {
    FATAL_ERROR("Error %d while reading TreeNHalos attribute from file '%s'", status, buf);
  }
...
{ \
  ...
  if (status != EXIT_SUCCESS) \
  {\
    FATAL_ERROR("Failed to read dataset %s for tree %d", dataset_name, treenr);\
  }\
...
{ \
  ...
  if (status != EXIT_SUCCESS) \
  {\
    FATAL_ERROR("Failed to read multi-dimension dataset %s for tree %d", dataset_name, treenr);\
  }\
  for (halo_idx = 0; halo_idx < NHalos_ThisTree; ++halo_idx)\
  {\
...
void load_tree_hdf5(int32_t filenr, int32_t treenr)
{
  ...
  if (hdf5_file <= 0)
  {
    FATAL_ERROR("The HDF5 file should still be opened when reading the halos in tree %d. Error code: %d", 
               treenr, hdf5_file);
  }
  ...
  if (buffer == NULL)
  {
    FATAL_ERROR("Could not allocate memory for the HDF5 buffer for tree %d (%d halos, %zu bytes)", 
  ...
  {
    FATAL_ERROR("Could not allocate memory for the HDF5 multiple dimension buffer for tree %d (%d halos, %zu bytes)", 
...
int32_t fill_metadata_names(struct METADATA_NAMES *metadata_names, enum Valid_TreeTypes my_TreeType)
{
  ...
  switch (my_TreeType)
  {
    ...
    default:
      FATAL_ERROR("Tree type %d has not been included in the switch statement for fill_metadata_names in io_tree_hdf5.c. Please add it there.", my_TreeType);
