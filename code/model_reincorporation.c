 ...
 * escape velocity exceeds the characteristic velocity of supernova-driven winds.
 */
 ...
 * from the galaxy due to supernova feedback. The reincorporation rate depends on:
 * 
 ...
 * supernova-driven winds. The rate increases for more massive halos (higher Vvir).
 * 
...
void reincorporate_gas(int centralgal, double dt)
{
   ...
   * SN velocity is ~630km/s, and reincorporation occurs when the halo escape velocity
   * exceeds this value. The escape velocity relates to virial velocity as V_esc = sqrt(2)*V_vir,
   * so the critical virial velocity is V_SN/sqrt(2) = 445.48 km/s
   * This is modified by a tunable parameter ReIncorporationFactor */
  ...
  if(is_greater(Gal[centralgal].Vvir, Vcrit))
  {
    ...
    Gal[centralgal].EjectedMass -= reincorporated;  /* Remove from ejected reservoir */
    Gal[centralgal].MetalsEjectedMass -= metallicity * reincorporated;  /* Remove metals */
    
    Gal[centralgal].HotGas += reincorporated;  /* Add to hot gas reservoir */
    Gal[centralgal].MetalsHotGas += metallicity * reincorporated;  /* Add metals */
  }
}
