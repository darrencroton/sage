...
#define IO_DEBUG_LOG(code, op, filename, ...)   log_io_error(LOG_LEVEL_DEBUG, code, __FILE__, __FUNCTION__, __LINE__, op, filename, __VA_ARGS__)
#define IO_INFO_LOG(code, op, filename, ...)    log_io_error(LOG_LEVEL_INFO, code, __FILE__, __FUNCTION__, __LINE__, op, filename, __VA_ARGS__)
#define IO_WARNING_LOG(code, op, filename, ...) log_io_error(LOG_LEVEL_WARNING, code, __FILE__, __FUNCTION__, __LINE__, op, filename, __VA_ARGS__)
#define IO_ERROR_LOG(code, op, filename, ...)   log_io_error(LOG_LEVEL_ERROR, code, __FILE__, __FUNCTION__, __LINE__, op, filename, __VA_ARGS__)
#define IO_FATAL_ERROR(code, op, filename, ...) do { \
    log_io_error(LOG_LEVEL_FATAL, code, __FILE__, __FUNCTION__, __LINE__, op, filename, __VA_ARGS__); \
    myexit(1); \
} while(0)
