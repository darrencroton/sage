...
class SnapshotRedshiftMapper:
    ...
    def __init__(self, param_file=None, params=None, output_dir=None):
        ...
        self.file_name_galaxies = params.get('FileNameGalaxies', 'model') if params else 'model'
    ...
    def get_model_file_path(self, snapshot, file_num):
        ...
        if self.output_dir:
            return os.path.join(self.output_dir, f"{self.file_name_galaxies}{redshift_str}_{file_num}")
        else:
    ...
    def debug_info(self):
        ...
        for i, (snap, z) in enumerate(zip(self.snapshots, self.redshifts)):
            if i >= 10:
                ...
                break
            info.append(f"  Snapshot {snap}: z={z:.3f}, file pattern={self.redshift_file_patterns[i]}")
