...
void *mymalloc_cat(size_t size, MemoryCategory category) {
    ...
    if (!ptr) {
        FATAL_ERROR("Memory allocation failed: Unable to allocate %.2f MB", alloc_size / (1024.0 * 1024.0));
    }
...
void *mymalloc(size_t size) {
    return mymalloc_cat(size, MEM_UNKNOWN);
}
...
void *myrealloc_cat(void *p, size_t size, MemoryCategory category) {
    /* Handle NULL pointer case */
    if (p == NULL) return mymalloc_cat(size, category);
    ...
    if (newp == NULL) {
        FATAL_ERROR("Memory reallocation failed: Unable to reallocate %.2f MB", alloc_size / (1024.0 * 1024.0));
    }
...
void *myrealloc(void *p, size_t size) {
    if (p == NULL) return mymalloc(size);
...
void myfree(void *p) {
    ...
    /* Optional detailed reporting */
    if (MemoryReportLevel >= MEMORY_REPORT_DETAILED) {
        INFO_LOG("Freed %.2f KB from category %s", 
                SizeTable[index] / 1024.0, CategoryNames[CategoryTable[index]]);
    }
...
void check_memory_leaks(void) {
    if (Nblocks > 0) {
        ...
        if (MemoryReportLevel >= MEMORY_REPORT_DETAILED) {
            for (unsigned long i = 0; i < Nblocks; i++) {
                WARNING_LOG("  Leaked block %lu: %.2f KB at %p (category: %s)", 
                           i, SizeTable[i] / 1024.0, Table[i], CategoryNames[CategoryTable[i]]);
            }
        }
    } else {
...
int validate_all_memory(void) {
    ...
    for (unsigned long i = 0; i < Nblocks; i++) {
        ...
        if (check_memory_guards(Table[i], SizeTable[i])) {
            ...
            WARNING_LOG("Memory corruption detected: Guards damaged for block %lu at %p", 
                       i, Table[i]);
            invalid_blocks++;
        }
#else
