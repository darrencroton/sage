...
def plot(galaxies, volume, metadata, params, output_dir="plots", output_format=".png", verbose=False):
    ...
    if len(valid_galaxies) == 0:
        ...
        ax.text(0.5, 0.5, "No galaxies found with Mvir > 0", 
    ...
    ax.plot(bin_centers[valid_bins], mean_occupation_all[valid_bins], 'k-', label='All galaxies')
    
    # Plot mean occupation for central galaxies (should be close to 1)
    ax.plot(bin_centers[valid_bins], mean_occupation_central[valid_bins], 'b--', label='Central galaxies')
    
    # Plot mean occupation for satellite galaxies
    ax.plot(bin_centers[valid_bins], mean_occupation_satellite[valid_bins], 'r-.', label='Satellite galaxies')
