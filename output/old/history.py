...
class Results:
    ...
    def read_gals(self, model_name, first_file, last_file):
        ...
        Galdesc_full = [
            ...
            ('ProgenitorGalaxyIndex'        , np.int64),                    
            ('HaloIndex'                    , np.int32),                    
            ('FOFHaloIdx'                   , np.int32),                    
            ('TreeIdx'                      , np.int32),                    
            ('SnapNum'                      , np.int32),                    
...
if __name__ == '__main__':
    ...
    # while((G_history[snap].ProgenitorGalaxyIndex[Index] >= 0) & (snap > FirstSnap)):
    while((G_history[snap].GalaxyIndex[Index] >= 0) & (snap > FirstSnap)):
      
      print snap, Index, G_history[LastSnap].TreeIdx[galaxyIndex], G_history[snap].TreeIdx[Index], G_history[snap].HaloIndex[Index], G_history[snap].GalaxyIndex[Index], G_history[snap].ProgenitorGalaxyIndex[Index], '  GAL: ', G_history[snap].Type[Index], np.log10(G_history[snap].Mvir[Index] * 1.0e10), np.log10(G_history[snap].StellarMass[Index] * 1.0e10)
        
      IndexTemp = np.where(G_history[snap].ProgenitorGalaxyIndex[Index] == G_history[snap-1].GalaxyIndex)[0]

      if(len(IndexTemp) > 1):print 'WARINING - non-unique progenitor for galaxy with index', galaxyIndex, len(IndexTemp)
      if(len(IndexTemp) == 0): print 'WARINING - unexpected lack of progenitor for galaxy with index', galaxyIndex
  
      if(len(IndexTemp)>1):
        print snap, IndexTemp[1], G_history[LastSnap].TreeIdx[galaxyIndex], G_history[snap].TreeIdx[IndexTemp[1]], G_history[snap].HaloIndex[IndexTemp[1]], G_history[snap].GalaxyIndex[IndexTemp[1]], G_history[snap].ProgenitorGalaxyIndex[IndexTemp[1]], '  GAL: ', G_history[snap].Type[IndexTemp[1]], np.log10(G_history[snap].Mvir[IndexTemp[1]] * 1.0e10), np.log10(G_history[snap].StellarMass[IndexTemp[1]] * 1.0e10)
