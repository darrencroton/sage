...
void deal_with_galaxy_merger(int p, int merger_centralgal, int centralgal, double time, double dt, int halonr, int step)
{
  ...
  /* Grow black hole through accretion from cold disk during mergers (Kauffmann & Haehnelt 2000) */
...
void grow_black_hole(int merger_centralgal, double mass_ratio)
{
  ...
  if(is_greater(Gal[merger_centralgal].ColdGas, 0.0))
  {
    ...
    Gal[merger_centralgal].ColdGas -= BHaccrete;        /* Remove gas that was accreted */
    Gal[merger_centralgal].MetalsColdGas -= metallicity * BHaccrete;  /* Remove corresponding metals */
 ...
 *
 * This function models the powerful winds generated during quasar-mode black hole
 * accretion. It compares the energy released by the quasar wind to the binding
 * energies of the galaxy's gas components, and removes gas that can be ejected
 * by the wind.
 * 
 ...
 *    is also ejected
 * 
 * The quasar energy is calculated as E = η × 0.1 × Maccrete × c², where η is the
 * quasar mode efficiency parameter and 0.1 is the assumed radiative efficiency.
 */
...
void add_galaxies_together(int t, int p)
{
   ...
   * Both disk and bulge components from the satellite are added to the bulge of the central */
 ...
 *
 * This function implements the morphological transformation from a disk-dominated
 * galaxy to a bulge-dominated one, typically triggered by a major merger.
 * It transfers all stellar content from the disk to the bulge component
 * and updates the star formation history arrays accordingly.
 * 
...
void make_bulge_from_burst(int p)
{
  ...
  Gal[p].BulgeMass = Gal[p].StellarMass;  /* Set bulge mass to total stellar mass */
 ...
 * @param   dt                 Time step size
 * @param   halonr             Index of the current halo
 * @param   mode               Burst mode: 0 for mergers, 1 for disk instabilities
 * @param   step               Current time integration step
 *
 * This function implements the collisional starburst model from Somerville et al. (2001)
 * with updated coefficients from TJ Cox's PhD thesis. It calculates the fraction
 * of cold gas that is converted to stars during a merger or disk instability event.
 *
...
void collisional_starburst_recipe(double mass_ratio, int merger_centralgal, int centralgal, double time, double dt, int halonr, int mode, int step)
{
   ...
   * The coefficients in eburst are taken from TJ Cox's PhD thesis and should be more accurate than previous. */
  ...
  if(SageConfig.SupernovaRecipeOn == 1)
    reheated_mass = SageConfig.FeedbackReheatingEpsilon * stars;  /* Proportional to star formation */
  else
  ...
  Gal[merger_centralgal].SfrBulgeColdGas[step] += Gal[merger_centralgal].ColdGas;  /* Record available gas */
  Gal[merger_centralgal].SfrBulgeColdGasMetals[step] += Gal[merger_centralgal].MetalsColdGas;  /* Record available metals */
   ...
   * Note: Disk instability is only checked for minor mergers (mass_ratio < threshold)
   * since major mergers already transform the remnant to a spheroid */
  ...
  /* Create new metals from the starburst (instantaneous recycling approximation)
   * Only considering Type II supernovae enrichment */
  if(Gal[merger_centralgal].ColdGas > 1e-8 && mass_ratio < SageConfig.ThreshMajorMerger)
  {
    /* Calculate fraction of metals that leave the disk (Krumholz & Dekel 2011, Eq. 22) 
     * In massive halos (Mvir >> 30), more metals are retained in the cold gas */
    ...
    Gal[merger_centralgal].MetalsColdGas += SageConfig.Yield * (1.0 - FracZleaveDiskVal) * stars;  /* Metals retained in cold gas */
    Gal[centralgal].MetalsHotGas += SageConfig.Yield * FracZleaveDiskVal * stars;  /* Metals ejected to hot gas */
...
/**
 * @brief   Disrupts a satellite galaxy, transferring its stars to intracluster stars
 *
 * @param   centralgal   Index of the central galaxy of the halo
...
{  
  ...
  Gal[centralgal].HotGas += Gal[gal].ColdGas + Gal[gal].HotGas;  /* All gas becomes hot */
