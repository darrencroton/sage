...
static int get_github_commit_url(char *url_buffer, size_t size, const char *hash) {
    ...
    if (execute_command("git config --get remote.origin.url 2>/dev/null", remote_url, sizeof(remote_url)) != 0) {
        return 1;
    }
...
#ifdef __GNUC__
    snprintf(compiler_buffer, size, "gcc %d.%d.%d", 
             __GNUC__, __GNUC_MINOR__, __GNUC_PATCHLEVEL__);
#elif defined(__clang__)
    snprintf(compiler_buffer, size, "clang %d.%d.%d", 
