...
def plot(galaxies, volume, metadata, params, output_dir="plots", output_format=".png", verbose=False):
    ...
    if not np.any(central_mask):
        ...
        ax.text(0.5, 0.5, "No central galaxies found with Mvir > 0", 
    ...
    for i in range(nbins - 1):
        ...
        for j in centrals_in_bin:
            ...
            halo_galaxies = np.where(galaxies.CentralGalaxyIndex == galaxies.CentralGalaxyIndex[j])[0]
    ...
    ax.axhline(y=baryon_frac, color='k', linestyle=':', lw=1.5, label=f'Cosmic: {baryon_frac:.2f}')
