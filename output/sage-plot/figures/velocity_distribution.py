...
def plot(galaxies, volume, metadata, params, output_dir="plots", output_format=".png"):
    ...
    if not np.any(valid_galaxies):
        ...
        ax.text(0.5, 0.5, "No galaxies found with valid positions", 
    ...
    # Line-of-sight velocity
    counts_los, binedges = np.histogram(vel_los / (hubble_h * 100.0), range=(bin_min, bin_max), bins=nbins)
    bin_centers = binedges[:-1] + bin_width / 2
    ax.plot(bin_centers, counts_los / bin_width / tot_gals, 'k-', lw=2, label='line-of-sight')
