 ...
 * with format-specific implementations in separate files.
 */
...
void load_tree_table(int filenr, enum Valid_TreeTypes my_TreeType)
{
  ...
  switch (my_TreeType)
  {
    ...
    default:
      FATAL_ERROR("Unsupported tree type %d in load_tree_table(). Please add support in io_tree.c", my_TreeType);
  }
...
void free_tree_table(enum Valid_TreeTypes my_TreeType)
{
  ...
  switch (my_TreeType)
  {
    ...
    default:
      FATAL_ERROR("Unsupported tree type %d in free_tree_table(). Please add support in io_tree.c", my_TreeType);
...
void load_tree(int filenr, int treenr, enum Valid_TreeTypes my_TreeType)
{
  ...
  case genesis_lhalo_hdf5:
    ...
    default:
      FATAL_ERROR("Unsupported tree type %d in load_tree(). Please add support in io_tree.c", my_TreeType);
