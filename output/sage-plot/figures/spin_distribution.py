...
def plot(galaxies, volume, metadata, params, output_dir="plots", output_format=".png", verbose=False):
    ...
    valid_galaxies = np.where((galaxies.Vvir > 0.0) & (galaxies.Rvir > 0.0) & 
                             (galaxies.Spin[:,0]**2 + galaxies.Spin[:,1]**2 + galaxies.Spin[:,2]**2 > 0.0))[0]
    
    # Check if we have any galaxies to plot
    if len(valid_galaxies) == 0:
        print("No valid galaxies found for spin distribution plot")
        # Create an empty plot with a message
        ax.text(0.5, 0.5, "No valid galaxies found for spin distribution plot", 
    ...
    spin_magnitude = np.sqrt(galaxies.Spin[valid_galaxies,0]**2 + 
                            galaxies.Spin[valid_galaxies,1]**2 + 
                            galaxies.Spin[valid_galaxies,2]**2)
    
    spin_parameter = spin_magnitude / (np.sqrt(2) * galaxies.Vvir[valid_galaxies] * galaxies.Rvir[valid_galaxies])
    
    # Filter out any invalid values
    valid_spins = np.where((spin_parameter > 0.0) & (spin_parameter < 1.0) & 
                           np.isfinite(spin_parameter))[0]
    
    if len(valid_spins) == 0:
        print("No valid spin parameter values found")
        # Create an empty plot with a message
        ax.text(0.5, 0.5, "No valid spin parameter values found", 
                horizontalalignment='center', verticalalignment='center',
                transform=ax.transAxes, fontsize=IN_FIGURE_TEXT_SIZE)
    ...
    p_lognormal = (1/(x*sigma*np.sqrt(2*np.pi))) * np.exp(-np.log(x/lambda_0)**2/(2*sigma**2))
